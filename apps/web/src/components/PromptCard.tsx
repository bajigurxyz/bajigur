 ...
 *
 * Bajigur prompts are text, so there is no media preview — the `preview` line
 * is what a buyer judges before paying, which is exactly what the catalogue
...
export default function PromptCard({ prompt }: { prompt: Prompt }) {
  ...
  return (
    ...
    >
      <PromptPreview src={prompt.previewMedia} title={prompt.title} className="h-28" />
      ...
      <p className="text-sm leading-relaxed text-gray-600">{prompt.preview}</p>
