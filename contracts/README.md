...
`BajigurRegistrar` lets any wallet claim one `<label>.bajigur.eth` for free with
its own resolver. Deploy (grants `ROLE_REGISTRAR` on the subregistry) and claim:
...
`axel.bajigur.eth` was claimed this way by `0xE5d8…e950`, which then set its own
`bajigur.hedera` record; the API pays that creator from it.
